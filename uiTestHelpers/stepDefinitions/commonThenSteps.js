...
Then(/^I expect (?: the)? '([^']*)?' to be (visable|hidden)$/, checkVisibility);
...
Then(/^I expect(?: the)? '([^']*)?' to( not)* be checked$/, checkIsSelected);
Then(/^I expect(?: the)? '([^']*)?' to( not)* be selected$/, checkIsSelected);
