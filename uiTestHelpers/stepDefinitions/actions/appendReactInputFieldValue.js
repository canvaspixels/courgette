...
module.exports = async function appendReactInputFieldValue(text, locatorKey) {
  const fnToExecInBrowser = (domEl, val) => {
  ...
  const el = await this.getCurrentPage().getElementWhenInDOM(locatorKey)
  if (process.env.BINDINGS === 'WDIO') {
    await browser.execute(fnToExecInBrowser, el, text);
  } else {
    await browser.executeScript(fnToExecInBrowser, el, text);
  }
};
